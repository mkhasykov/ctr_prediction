...
def focalloss(p, y, gamma=2.):
    p = max(min(p, 1. - 10e-15), 10e-15)
    return -(1. - p)**gamma * log(p) if y == 1. else -p**gamma * log(1. - p)


def grad_focal(p, y, gamma=2.):
    p = max(min(p, 1. - 10e-15), 10e-15)
    # g1 = p * (1 - p)
    g2 = y + (-1)**y * p
    g3 = p + y - 1
    g4 = 1 - y - (-1)**y * p
    g5 = y + (-1)**y * p
    return gamma * g3 * g2**gamma * log(g4) + (-1)**y * g5**(gamma + 1)
